...
$ScriptBlock = {
    ...
    function getWinEvtWhere($LogName, $XPath, $SecondaryFilter) {
        $DebugPreference = "Continue"
        Write-Debug ($PSBoundParameters |out-string)
        Write-Debug $XPath.GetType()
        if (Test-Path $LogName) { $params = @{Path=$Logname;MaxEvents=1} }
        else { $params = @{LogName=$LogName;MaxEvents=1} }
        if (-Not ([string]::IsNullOrEmpty($XPath))) { $params.Add('FilterXPath', $XPath) }
        if ($SecondaryFilter) { Get-WinEvent @params | Where-Object $SecondaryFilter }
    ...
    $sb = {
        ...
        getWinEvtWhere @args | Get-WinEventData |
            Select-Object TimeCreated,MachineName,LogName,ContainerLog,Data_*
    }

    Write-Verbose "Searching live log..."
    &$sb $using:LogName $using:XPath $using:SecondaryFilter
    ...
    if ($using:ArchiveNewerThan) {
        ...
        foreach ($log in $archives) {
            ...
            &$sb $log $using:XPath $using:SecondaryFilter
        }
    }
}
